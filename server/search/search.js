...
module.exports = {
  search: ( req, res, next ) => {
    ...
    db.query(s)
      .then( (response) => {
        var properties = response.slice();//makes a coppy of the array
  ...
  emailSearch: async function ( item, db ) {

    //unpack the req object.
    let { location, ammount, cash_deal, move_in, rate } = item; 
    ...
    // db.get_properties([ location, locationType, listPriceMax ])
    let response = await db.query(s).then(r => r).catch(console.log)
    let properties = response.slice();//makes a coppy of the array
    console.log(properties)

    for(let i = 0; i < properties.length; i++){
  
      let { city, list_price, state, street_name, street_number, street_suffix } = properties[i]; //unpack address data for the home

      street_name = street_name.replace(/ /g, '+'); 
      city = city.replace(/ /g, '+');
          
      //ping zillow for their rent estamate and link back to Zillow, set it to the property object
      let house = await axios.get(`http://www.zillow.com/webservice/GetSearchResults.htm?zws-id=${process.env.ZWS_ID}&address=${street_number}+${street_name}+${street_suffix}&citystatezip=${city}%2C+${state}&rentzestimate=true`).then(r => r).catch(console.log)
